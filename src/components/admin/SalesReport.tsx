...
export function SalesReport({ orders }: SalesReportProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const reservedOrders = filteredOrders.filter(o => o.status === 'reserved');
    ...
    const reservedRevenue = reservedOrders.reduce((sum, order) => sum + (order.reservationFee || 0), 0);
    ...
    const productsSold = completedOrders.reduce((sum, order) =>
    ...
    // Daily sales
    const dailySales = completedOrders.reduce((acc, order) => {
  ...
  const generatePDF = () => {
    ...
    const summaryMetrics = [
      ['Total Revenue', `₱${metrics.totalRevenue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`],
      ...
      ['Average Order Value', `₱${metrics.averageOrderValue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`],
    ...
    doc.save(fileName);
    toast.success('Professional sales report generated! 📊', {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Generate comprehensive sales reports for analysis</CardDescription>
          </div>
        </div>
      ...
      <CardContent>
        <div className="py-4">
          <p className="text-sm text-muted-foreground text-center">
            Select a date range and click "Generate Report" to create a comprehensive PDF document with all key metrics, top selling products, and daily sales breakdown.
          </p>
        </div>
      </CardContent>
    </Card>
