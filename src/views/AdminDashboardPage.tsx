...
export function AdminDashboardPage({ onNavigate }: AdminDashboardPageProps) {
  const { user, orders, updateOrderStatus, cancelOrder, processManualRefund, isAdmin, isOwner, hasAdminAccess, placeOrder, logout } = useAuth();
  ...
  const stats = useMemo(() => {
    ...
    const productSales: Record<number, { product: Product; quantity: number; revenue: number }> = {};
  ...
  const handleUpdateProduct = (id: number, productUpdates: Partial<Product>) => {
    ...
    if (productUpdates.name && oldProduct && productUpdates.name !== oldProduct.name) {
      changes.push({ field: 'name', oldValue: oldProduct.name, newValue: productUpdates.name });
    }
    if (productUpdates.price !== undefined && oldProduct && productUpdates.price !== oldProduct.price) {
      changes.push({ field: 'price', oldValue: `₱${oldProduct.price}`, newValue: `₱${productUpdates.price}` });
    }
    if (productUpdates.category && oldProduct && productUpdates.category !== oldProduct.category) {
      changes.push({ field: 'category', oldValue: oldProduct.category, newValue: productUpdates.category });
    }
    if (productUpdates.active !== undefined && oldProduct && productUpdates.active !== oldProduct.active) {
      changes.push({ 
        field: 'status', 
    ...
    const actionType = productUpdates.active !== undefined && !productUpdates.active 
      ? 'product_deleted' // When deactivating
      : productUpdates.active !== undefined && productUpdates.active && oldProduct && !oldProduct.active
  ...
  const [createdOrderForInvoice, setCreatedOrderForInvoice] = useState<Order | null>(null);
  const [showInvoiceDialog, setShowInvoiceDialog] = useState(false);

  const handleCreateManualOrder = (orderData: Omit<Order, 'id' | 'userId' | 'createdAt' | 'updatedAt' | 'status'>, customerUserId?: string) => {
      ...
      <div className="container max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-16">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <ImageWithFallback src={poybashLogo} alt="PoyBash Furniture" className="w-10 h-10" />
            <div>
              <h1>{getDashboardTitle(user?.role || 'admin')}</h1>
              <p className="text-muted-foreground">{getDashboardGreeting(user?.role || 'admin', user?.firstName || 'User')}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${[
            ...
            {user?.role === 'staff' && (
              <TabsTrigger value="sales-dashboard">
                Dashboard
              </TabsTrigger>
            )}
            {/* Inventory Dashboard - Only for Inventory Clerk */}
            {user?.role === 'inventory-clerk' && (
              <TabsTrigger value="inventory-dashboard">
                Dashboard
              </TabsTrigger>
            )}
            {hasPermission(user?.role || 'customer', 'view:products') && (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
              ...
              >
                ...
                <CardContent className="pt-6">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <p className="text-muted-foreground text-sm mb-1">NEW CUSTOMERS</p>
                  ...
                  <div className="h-16 w-full min-h-[64px]">
                    <ResponsiveContainer width="100%" height={64} minWidth={0}>
                      <LineChart data={stats.dailyData}>
                        <Line type="monotone" dataKey="completedOrders" stroke="#deb887" strokeWidth={2} dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">Revenue trend over the last 7 days</CardDescription>
                </CardHeader>
                  ...
                  <div className="h-64 min-h-[256px]">
                    <ResponsiveContainer width="100%" height="100%" minHeight={256}>
                      <AreaChart data={stats.dailyData}>
                        ...
                        />
                        <Area type="monotone" dataKey="revenue" stroke="#8b4513" strokeWidth={2} fillOpacity={1} fill="url(#colorRevenue)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Best performing furniture by revenue</CardDescription>
                </CardHeader>
                <CardContent>
                      ...
                      stats.topProducts.map((item, index) => (
                        <div key={item.product.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-secondary/30 transition-colors">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-muted-foreground">{item.product.category} • {item.quantity} sold</p>
                          </div>
                ...
                <div className="bg-gradient-to-br from-[#f5deb3]/20 to-[#deb887]/10 p-8 rounded-xl border border-[#deb887]/30">
                  <div className="max-w-4xl mx-auto">
                    <div className="mb-6">
                      ...
                      <p className="text-muted-foreground">Generate comprehensive reports for business insights</p>
                    </div>
          ...
          <TabsContent value="orders" className="space-y-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setShowQRScanner(!showQRScanner)} className="gap-2">
                  <QrCode className="h-4 w-4" />
                  {showQRScanner ? 'Hide Scanner' : 'Scan QR Code'}
                </Button>
                <CreateOrderDialog onCreateOrder={(orderData, customUserId) => {
            ...
            {/* QR Scanner */}
            {showQRScanner && (
              <QRScanner orders={orders} onClose={() => setShowQRScanner(false)} />
            )}
          ...
          <TabsContent value="inventory" className="space-y-6 pb-12">
            <InventoryTracking products={products} onRefresh={refreshProducts} />
          </TabsContent>
        ...
        <Dialog open={showInvoiceDialog} onOpenChange={setShowInvoiceDialog}>
          <DialogContent className="max-w-6xl max-h-[95vh]">
            ...
            <div className="overflow-y-auto max-h-[calc(95vh-120px)] invoice-scroll pb-8">
              {createdOrderForInvoice && <InvoiceReceipt order={createdOrderForInvoice} />}
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {showInvoice ? (
        ...
        <div className="space-y-6">
              ...
              <div key={index} className="flex gap-4 p-4 border rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  {item.refundRequested && (
                    <Badge 
                      variant={item.refundStatus === 'pending' ? 'secondary' : item.refundStatus === 'approved' ? 'default' : 'destructive'}
                      className="flex items-center gap-1 w-fit mt-2"
                    >
                      {item.refundStatus === 'pending' && <Clock className="h-3 w-3" />}
                      {item.refundStatus === 'approved' && <CheckCircle className="h-3 w-3" />}
                      {item.refundStatus === 'rejected' && <XCircle className="h-3 w-3" />}
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 bg-secondary/30 rounded-lg space-y-2">
              <p><strong>Method:</strong> {order.deliveryMethod === 'store-pickup' ? 'Store Pickup' : 'Customer Arranged Delivery'}</p>
              {order.pickupDetails && (
                <>
                  <p><strong>Contact Person:</strong> {order.pickupDetails.pickupPerson}</p>
                  <p><strong>Contact Phone:</strong> {order.pickupDetails.pickupPhone}</p>
                  {order.pickupDetails.deliveryService && (
                    <p><strong>Delivery Service:</strong> {order.pickupDetails.deliveryService}</p>
                  )}
                </>
              )}
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 bg-secondary/30 rounded-lg space-y-2">
              <p><strong>Method:</strong> {order.paymentMethod === 'cash' ? 'Cash on Pickup' : order.paymentMethod === 'gcash' ? 'GCash' : 'Bank Transfer'}</p>
              {order.paymentReference && (
                <p><strong>Reference Number:</strong> {order.paymentReference}</p>
              )}
              {order.paymentProof && (
                <div>
                  <p><strong>Payment Proof:</strong></p>
                  <img src={order.paymentProof} alt="Payment proof" className="mt-2 max-w-xs rounded border" />
                </div>
              )}
